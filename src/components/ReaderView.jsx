      ...
      <div className="space-y-6 animate-in slide-in-from-right-4 fade-in pb-20">
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <span className="text-xs font-bold text-orange-600 bg-orange-50 dark:bg-orange-900/30 px-2 py-1 rounded-full">{formatGenreTag(book.category)}</span>
            {(book.genre || book.subCategory) && <span className="text-xs font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-full">{formatGenreTag(book.genre || book.subCategory)}</span>}
          </div>
          <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 leading-tight">{book.title}</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500">{book?.isAnonymous ? '익명' : (book.authorNickname || '익명')} · {formatDate(book.createdAt, book.dateKey)}</p>
        </div>
        <div className={`prose prose-slate max-w-none ${fontSize} leading-relaxed text-slate-700 dark:text-slate-300 whitespace-pre-line bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm`}>
          {book.content}
        </div>
      </div>
    );
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 fade-in pb-20">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          {currentStory.episode > 1 && <span className="text-xs font-bold text-slate-400 dark:text-slate-500">#{currentStory.episode}화</span>}
        </div>
        <div className="flex items-center gap-1">
          ...
          <button onClick={handleShare} className="p-2 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700"><Share2 className="w-5 h-5 text-slate-400" /></button>
          <button onClick={() => setIsReportModalOpen(true)} className="p-2 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700"><AlertTriangle className="w-5 h-5 text-slate-400" /></button>
        </div>
      </div>
      ...
      {showTranslateMenu && (
        <div className="flex justify-end gap-2 mb-4 animate-in fade-in slide-in-from-top-2">
          <button onClick={() => translateStory('ko')} className={`px-3 py-1.5 rounded-full text-xs font-bold border ${readerLang === 'ko' ? 'bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 border-slate-800 dark:border-slate-200' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600'}`}>한국어</button>
          <button onClick={() => translateStory('en')} className={`px-3 py-1.5 rounded-full text-xs font-bold border ${readerLang === 'en' ? 'bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 border-slate-800 dark:border-slate-200' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600'}`}>English</button>
        </div>
      )}

      {/* Content */}
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm space-y-4 min-h-[50vh]">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {comments.map((comment) => (
            <div key={comment.id} className={`bg-white dark:bg-slate-800 p-4 rounded-xl border ${comment.userId === user.uid ? 'border-orange-100 dark:border-orange-900/50 bg-orange-50/10 dark:bg-orange-900/10' : 'border-slate-100 dark:border-slate-700'} space-y-2`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[10px] text-slate-400 dark:text-slate-500">{comment.createdAt?.toDate ? comment.createdAt.toDate().toLocaleDateString() : 'Now'}</span>
                    </div>
                  </div>
                </div>
