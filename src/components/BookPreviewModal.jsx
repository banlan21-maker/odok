...
const BookPreviewModal = ({ book, user, userProfile, useItem, onRead, onClose, onGoToStore, authorProfiles = {} }) => {
  ...
  const handleGenerateSummary = async (type) => {
    ...
    try {
      ...
      setError(err.message || '소개글 생성에 실패했습니다.');
      setPhase('view');
    }
  };
            ...
            <div className="flex gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {book.isAnonymous ? '익명' : (authorProfiles[book.authorId]?.nickname || book.authorNickname || '알 수 없는 작가')}
                </p>
                {book.genre && (
            ...
            <div className="mb-4">
              ...
              {phase === 'generating' && (
                <div className="bg-sky-50 dark:bg-sky-950/30 rounded-2xl px-4 py-5 text-center space-y-2">
                  ...
                  <p className="text-xs font-black text-sky-600 dark:text-sky-400">소개글 작성 중...</p>
                </div>
              )}
              ...
              {isVisible && summaryType === 'NONE' && (
                <div className="bg-slate-50 dark:bg-slate-700 rounded-2xl px-4 py-4 space-y-3">
                  <p className="text-xs text-slate-400 dark:text-slate-500 text-center">소개글이 없습니다.</p>
                  {isMyBook && (
                    <button
