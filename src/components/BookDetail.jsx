    ...
    <>
      <div className="animate-in slide-in-from-right-4 fade-in pb-20">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm space-y-4">
          <h3 className="text-sm font-bold text-slate-700">{(t?.comment_count || "댓글 {count}개").replace('{count}', comments.length)}</h3>
          <div className="space-y-3 max-h-80 overflow-auto pr-1">
            {comments.length === 0 ? (
              <p className="text-xs text-slate-400">{t?.first_comment || "첫 댓글을 남겨보세요."}</p>
            ) : (
              comments.map((c) => {
                  ...
                  >
                    ...
                    {!isEditing && (
                      <div className="flex items-center gap-2 mt-2 text-xs text-slate-500">
                        <button onClick={() => startReply(c)} className="hover:text-slate-700">{t?.reply || "답글"}</button>
                        {isMine && (
                          <>
                            <span>·</span>
          ...
          {replyTo && (
            <div className="text-xs text-slate-500">
              {(t?.reply_to || "{name}에게 답글 작성 중").replace('{name}', replyTo.authorName)} · <button onClick={cancelReply} className="text-slate-600 hover:text-slate-800">{t?.cancel || "취소"}</button>
            </div>
          )}
          <p className="text-xs text-slate-400">{t?.comment_limit || "댓글은 200자 이내로 작성하세요."}</p>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-6">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
              <div className="text-center space-y-2">
                <h3 className="text-lg font-black text-slate-800">{t?.series_continue_title || "다음 화 전개 방식"}</h3>
